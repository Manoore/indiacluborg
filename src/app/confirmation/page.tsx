...
function ConfirmationInner() {
  ...
  const { participants, getCommunity, campaign } = useData();
  ...
  const community = getCommunity(participant.communityId);
  const shareText = `❤️ I just joined ${campaign.teamName} for the Heart Walk on ${new Date(
    campaign.date
  ).toLocaleDateString("en-US", { month: "long", day: "numeric" })}! I'm walking with the ${
    community?.name ?? ""
  } to support heart health. Join me and add your heart!`;
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-14 text-center">
      ...
      <p className="mt-2 text-foreground/60">
        Welcome to {campaign.teamName}, {participant.firstName}!
      </p>
      ...
      >
        ...
        <p className="text-sm font-medium text-saffron-deep">{community?.name}</p>
      </motion.div>
      ...
      <Link
        href="/"
        className="mt-10 inline-block font-semibold text-heart underline-offset-4 hover:underline"
      >
