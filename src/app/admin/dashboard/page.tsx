...
export default function AdminDashboard() {
  ...
  const { participants, communityStats, totals, getCommunity, campaign } = useData();
  ...
  const sortedParticipants = useMemo(
    () => [...participants].sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1)),
    [participants]
  );
  ...
  function exportCsv() {
    ...
    const rows = sortedParticipants.map((p) => [
      ...
      p.email,
      p.phone ?? "",
      getCommunity(p.communityId)?.name ?? "",
      p.participationType,
        ...
        {tab === "participants" && (
          <div className="rounded-2xl border border-black/5 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-black/5 p-4">
              <h3 className="text-sm font-bold text-navy">Participants ({sortedParticipants.length})</h3>
              <button
                onClick={exportCsv}
                ...
                <tbody>
                  {sortedParticipants.map((p) => (
                    <tr key={p.id} className="border-b border-black/5 hover:bg-cream/60">
                      ...
                      <td className="px-4 py-2 text-foreground/70">{getCommunity(p.communityId)?.name}</td>
